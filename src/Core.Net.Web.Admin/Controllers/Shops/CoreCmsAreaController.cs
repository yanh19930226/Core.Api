...
namespace Core.Net.Web.Admin.Controllers.Shops
{
    ...
    public class CoreCmsAreaController : ControllerBase
    {
        ...
        public async Task<JsonResult> GetAllList()
        {
            ...
            var list = await _coreCmsAreaServices.QueryAsync();
            //返回数据
            jm.data = list;
            jm.code = 0;
            jm.msg = "数据调用成功!";
        ...
        public async Task<JsonResult> DoDelete([FromBody] FMIntId entity)
        {
            ...
            var res = await _coreCmsAreaServices.DeleteByIdAsync(entity.id);
            jm.code = res ? 0 : 1;
            jm.msg = res ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            return new JsonResult(jm);
        }
